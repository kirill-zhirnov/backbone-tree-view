...
BackTree.plugins.DnD = (function(superClass) {
  ...
  DnD.prototype.getOverlappedEl = function(el) {
    ...
    if (!this.doesElHaveView($el)) {
      ...
      if ($el.length !== 1 || !this.doesElHaveView($el)) {
        $el = false;
      }
    }
    return $el;
  ...
  DnD.prototype.shallLevelDown = function(coefficient) {
    ...
    if ($prev.length && this.doesElHaveView($prev) && coefficient.leftMargin > 30) {
      view = $prev.data('view');
  ...
  DnD.prototype.shallLevelUp = function(coefficient) {
    ...
    if (coefficient.leftMargin < -30) {
      ...
      if ($parent.length && this.doesElHaveView($parent)) {
        $children = $parent.find('> ul > .bt-item');
        if ($children.length > 0 && $children.index(this.$overlapped) === ($children.length - 1)) {
          return $parent;
        }
      }
    }
